...
class WorkflowBaselineTests(unittest.TestCase):
    ...
    def test_readme_is_chinese_single_file_with_core_commands(self) -> None:
        ...
        self.assertIn("编排层由项目内 tool-call loop 实现", content)
        self.assertIn("没有引入 LangChain / LangGraph", content)
    ...
    def _assert_shared_readme_structure(self, content: str) -> None:
        ...
        self.assertIn("架构地图", content)
        self.assertIn("当前能力", content)
        self.assertIn("设计边界", content)
        self.assertIn("路线图", content)
        ...
        self.assertIn("agent-app --workspace-root .", content)
        self.assertIn("python -m agent_app.cli --workspace-root .", content)
        ...
        self.assertIn("recent tool evidence replay", content)
